...
const register = async (req, res) => {
  try {
    ...
    // SECURITY: role is never taken from client input on public registration.
    // Public /register always creates a plain "student" account. Elevated
    // roles (faculty, hod, vc, iqac_director, staff, superadmin) can only be
    // created through the authenticated+authorized admin routes.
    const user = await User.create({
      name: data.name, email: data.email, phone: data.phone,
      password: hashedPassword, role: 'student', mustChangePassword: true
    });
