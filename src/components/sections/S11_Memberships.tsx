...
function PreviewCard({ m, onEdit, onDelete, disabled }: { m: any; onEdit: () => void; onDelete: () => void; disabled: boolean }) {
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div style={{ display: 'flex', gap: 16, flex: 1, cursor: 'pointer' }} onClick={() => setExpanded(!expanded)}>
          <div style={{ minWidth: 56, textAlign: 'center', padding: '6px 4px', borderRadius: 8, background: 'var(--primary, #2563eb)', flexShrink: 0 }}>
            <div style={{ fontSize: 14, fontWeight: 800, color: '#ffffff', lineHeight: 1 }}>{m.yearOfJoining || '—'}</div>
...
export default function Memberships({ data, onChange }: { data: any[]; onChange: (d: any[]) => void }) {
  ...
  const sortedData = [...data].sort((a, b) => (parseInt(b.yearOfJoining) || 0) - (parseInt(a.yearOfJoining) || 0));
  ...
  return (
    <>
      <div className="section-header-actions" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, gap: 16 }}>
        <h5 style={{ margin: 0, flex: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>Professional Memberships</h5>
        <button
          type="button"
          onClick={handleAdd}
      ...
      {sortedData.length === 0 && (
        <div className="empty-state">No memberships added yet. Click Add Membership to get started.</div>
      )}
