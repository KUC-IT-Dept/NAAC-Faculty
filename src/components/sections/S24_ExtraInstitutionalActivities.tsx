                  ...
                  <>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        ...
                        <button type="button" onClick={() => confirmDelete(async () => { const updated = responsibilities.filter((_: any, j: number) => j !== i); update(updated); setEditingIndex(null); setIsDirty(false); if (onPersist) await onPersist(updated, false); })} style={btnDelete}><Trash2 size={14} /> Delete</button>
                      </div>
                    </div>
